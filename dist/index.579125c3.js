    ...
    toggleTheme: document.getElementById('toggle').addEventListener('click', ()=>{
        ...
        if (state == true) {
            ...
            elements.forEach((item)=>{
                if (item === 'main' || item === 'form') document.getElementById(item).setAttribute('style', 'color: black; background-color: #d7d7d7; font-weight: bold');
...
projects.innerHTML = project.map((item1)=>`<div id=${item1.name} class=" bg-medium p-4 flex flex-col md:grid md:grid-cols-2 w-full md:justify-self-center md:place-self-center md:rounded-md">
                <span class="w-full">
                    <h2 class="text-xl font-semibold text-red-500">${item1.name}</h2>
                    ...
                    <ul class="grid grid-cols-4 md:flex w-full md:flex-row  gap-2">
                        ${item1.stack.map((item)=>`<li>${item}</li>`
    ).join(' ')}
                    </ul>
                    <span class="flex flex-row w-full justify-evenly md:justify-start md:gap-4 my-4">
                        <a href=${item1.demo} id='button' class="bg-red-600 shadow-light shadow py-2 font-bold px-4 rounded">Live Demo</a>
                        <a href=${item1.repo} id='button' class="bg-black py-2 px-4 shadow-light shadow font-bold rounded">Go To Repo</a>
                    </span>
                </span>
            </div>`
).join(' ');
